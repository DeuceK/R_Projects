public class Inode {
	...
	void toDisk( short iNumber ) {     
		// you implement
		int num = (iNumber / 16) + 1;
		byte[] data = new byte[Disk.blockSize];
		SysLib.rawread(num, data);
		int buff = (iNumber % 16) * iNodeSize;
		SysLib.int2bytes(length, data, buff);
		buff += 4;
		SysLib.short2bytes(count, data, buff);
		buff += 2;
		SysLib.short2bytes(flag, data, buff);
		buff += 2;

		for(int i = 0;i < directSize;i++){
			SysLib.short2bytes(direct[i], data, buff);
			buff += 2;
		}
		SysLib.short2bytes(indirect, data, buff);
